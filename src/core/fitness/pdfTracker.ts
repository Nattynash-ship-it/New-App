/**
 * The printable-tracker view of the 8-Week Glute & Sculpt program: transcribed
 * verbatim from public/plans/8-week-glute-sculpt-tracker.pdf, so each day lists
 * the PDF's exact exercises and sets × reps, Sunday is the PDF's REST day, and
 * completion is tracked as a W1–W8 grid mirroring the paper checkboxes.
 *
 * NOTE: this is deliberately separate from ./program.ts, which models the same
 * program a different way (a day-by-day schedule drawn from the app's workout
 * library, with a workout every day). Both trackers ship side by side and keep
 * their own state, so progress in one never mixes with the other:
 *   • this module  → store: `programProgress` / `programWeek` / `trackerStartDate`
 *   • ./program.ts → store: `programDone` / `programExtras` / `programStartDate`
 *
 * Static template — the *plan* never changes; progress lives in the store.
 */
